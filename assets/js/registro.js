...
function contrasenaNoCoincide(mensaje){
...
formulario.addEventListener("submit", function(evento){
    ...
    const anios = (fechaActual - fechaIngresada)/(1000 * 60 * 60 * 24 * 365.25)
    ...
    if(nombre.trim() === "" || apellido.trim() === "" || correo.trim() === "" || fechaNacimiento.trim() === "" || contrasena.trim() === "" || region.trim() === "" || direccion.trim() === ""){
        mostrarError("Todos los campos son obligatorios")
    }else{
        mostrarExito("Registro realizado con éxito")
        //agregar funcion de registrar usuario
        formulario.reset()
    }
})
